	...
	public class PlatformSettings_iOS : IPlatformSettings
	{
		...
		public void DrawSetting(SerializedObject serializedObject)
		{
			...
			using (new EditorGUIEx.GroupScope("iOS Settings"))
			{
				...
				// Signing.
				EditorGUILayout.LabelField(EditorLanguage.GetContent("Signing"), EditorStyles.boldLabel);
				var spAutomaticallySign = settings.FindPropertyRelative("automaticallySign");
				EditorGUI.indentLevel++;
				{
					EditorGUIEx.PropertyField(spAutomaticallySign);
					EditorGUIEx.PropertyField(settings.FindPropertyRelative("developerTeamId"));
					if (!spAutomaticallySign.boolValue)
					{
						EditorGUIEx.PropertyField(settings.FindPropertyRelative("codeSignIdentity"));
						EditorGUIEx.PropertyField(settings.FindPropertyRelative("profileId"));
						EditorGUIEx.PropertyField(settings.FindPropertyRelative("profileSpecifier"));
					}
				}
				EditorGUI.indentLevel--;
				...
				// exportOptions.plist.
				EditorGUILayout.LabelField(EditorLanguage.GetContent("exportOptions.plist Setting"), EditorStyles.boldLabel);
				EditorGUI.indentLevel++;
				{
					var spGenerate = settings.FindPropertyRelative("generateExportOptionPlist");
					EditorGUIEx.PropertyField(spGenerate, EditorLanguage.GetContent("Generate Automatically"));
					if (spGenerate.boolValue)
					{
						...
						EditorGUIEx.PropertyField(settings.FindPropertyRelative("uploadBitcode"));
						EditorGUIEx.PropertyField(settings.FindPropertyRelative("uploadSymbols"));
					}
				}
				EditorGUI.indentLevel--;
			}
